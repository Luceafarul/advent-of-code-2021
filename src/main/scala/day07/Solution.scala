 ...
 */
object Solution {
  def leastFuelSpendingForChangePosition(
      positions: List[Int],
      evaluationStrategy: (Int, Int, Int) => Int = spendingFuelConstantly
  ): Int = {
    @tailrec
    def loop(index: Int, fuelSpend: List[Int]): Int = {
      if (index == positions.size) fuelSpend.min
      else {
        val fuelSpending = positions.foldLeft(0) { (acc, elem) =>
          evaluationStrategy(acc, index, elem)
        }
        loop(index + 1, fuelSpend :+ fuelSpending)
      }
    }

    loop(index = 0, fuelSpend = List.empty)
  }

  val spendingFuelConstantly: (Int, Int, Int) => Int = (acc, index, elem) =>
    acc + math.abs(index - elem)

  val spendingFuelIncreasingCost: (Int, Int, Int) => Int =
    (acc, index, elem) => acc + (0 to math.abs(index - elem)).sum
}
