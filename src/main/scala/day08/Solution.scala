...
but those wires are connected to segments randomly. Worse, the wire/segment
...
and finally the four digit output value. Within an entry, the same wire/segment
...
Because 7 is the only digit that uses three segments, dab
in the above example means that to render a 7, signal lines d, a, and b are on.
...
output value. Unfortunately, in the above example, all of the digits in
the output value (cdfeb fcadb cdfeb cdbaf) use five segments and are more difficult to deduce.
...
object Solution {
  ...
  def decodedNumbers(input: Map[List[String], List[String]]): List[Int] = {
    val emptyString = ""
