...
each line is a measurement of the sea floor depth as the sweep looks further and
...
How many measurements are larger than the previous measurement? - 1292
...
then compare B with C, then C with D, and so on.
...
How many sums are larger than the previous sum? - 1262
 */
object Solution {
  def depthMeasurementIncreasesCounter(xs: List[Int]): Int =
    increasesMeasurementCounter(xs)

  def threeMeasurementSlidingWindowCounter(xs: List[Int]): Int =
    increasesMeasurementCounter(xs.sliding(3).map(_.sum).toList)

  private def increasesMeasurementCounter(xs: List[Int]): Int = {
    val (_, counter) = xs.foldLeft((0, 0)) { (acc, current) =>
