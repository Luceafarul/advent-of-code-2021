...
Each position is shown as the number of lines which cover that point or . (dot)
...
where at least two lines overlap. In the above example,
...
object Solution {
  ...
  final case class Path(x1: Int, y1: Int, x2: Int, y2: Int)

  def buildDiagram(
      paths: List[Path],
      filter: Path => Boolean = horizontalOrVertical
  ): Map[Coordinates, Int] = {
    paths
      .filter(filter)
      .flatMap(pathByCoordinates)
      .foldLeft(Map.empty[Coordinates, Int]) { (acc, elem) =>
  ...
  private def horizontalOrVertical(path: Path): Boolean =
    path.x1 == path.x2 || path.y1 == path.y2

  def overlapCount(diagram: Map[Coordinates, Int]): Int =
    diagram.values.count(_ >= 2)

  def pathByCoordinates(path: Path): List[Coordinates] = {
    val minX = path.x1 min path.x2
    val minY = path.y1 min path.y2

    val xs =
      if (path.x1 > path.x2) path.x1.to(path.x2, -1).toList
      else path.x1.to(path.x2).toList
    val ys =
      if (path.y1 > path.y2) path.y1.to(path.y2, -1).toList
      else path.y1.to(path.y2).toList

    xs.zipAll(ys, minX, minY)
      .map { case (x, y) =>
        Coordinates(x, y)
      }
  }
