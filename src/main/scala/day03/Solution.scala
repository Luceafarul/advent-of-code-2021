...
the most common bit in the corresponding position of all numbers in the diagnostic report.
...
than 1 bits (3), so keep only the 4 numbers
 ...
 */
object Solution {
  final case class BinaryNumber(number: String)

  def gammaRate(diagnosticReport: List[BinaryNumber]): BinaryNumber = {
    @tailrec
    def loop(report: List[BinaryNumber], acc: BinaryNumber): BinaryNumber = {
      if (report.forall(_.number.isEmpty)) acc
      else {
        val mcb = mostCommonBit(report)
        loop(
          report.map(bn => BinaryNumber(bn.number.drop(1))),
          acc.copy(number = acc.number + mcb)
        )
      }
    }

    loop(diagnosticReport, BinaryNumber(""))
  }

  def epsilonRate(diagnosticReport: List[BinaryNumber]): BinaryNumber =
    BinaryNumber(
      gammaRate(diagnosticReport).number
        .replaceAll("0", "x")
        .replaceAll("1", "0")
        .replaceAll("x", "1")
    )
  ...
  private def mostCommonBit(numbers: List[BinaryNumber]): Int = {
    val (oneCount, zeroCount) = numbers.foldLeft((0, 0)) { (acc, bn) =>
      ...
      bn.number.head match {
        case '0' => (oneCount, zeroCount + 1)
        case '1' => (oneCount + 1, zeroCount)
      }
    }
