...
Each octopus slowly gains energy over time and flashes brightly for
a moment when its energy is full. Although your lights are off,
maybe you could navigate through the cave without disturbing the octopuses
...
Adjacent flashes can cause an octopus to flash on a step even
if it begins that step with very little energy.
...
object Solution {
  ...
  final case class Grid(octopuses: List[Octopus]) {
    def update(octopus: Octopus): Grid = Grid {
      (octopuses.filterNot(c =>
        c.x == octopus.x && c.y == octopus.y
      ) :+ octopus)
    }
    def update(cs: List[Octopus]): Grid = Grid(cs)
    def notFlashed(): List[Octopus] = octopuses.filterNot(_.flashed)
    def flashed(): List[Octopus] = octopuses.filter(_.flashed)
  }

  object Grid {
    def of(values: Values): Grid = Grid {
      (for {
        p <- values
        x <- values.indices
        y <- p.indices
      } yield Octopus(x, y, values(x)(y))).distinct
    }
  }

  final case class Octopus(
      x: Int,
      y: Int,
      energyLevel: Int,
      flashed: Boolean = false
  ) {
    def increaseEnergyLevel: Octopus = this.copy(energyLevel = energyLevel + 1)
  ...
  def modelFlashes(grid: Grid, steps: Int): (Grid, Int) = {
    ...
    def loop(grid: Grid, steps: Int, totalFlashes: Int): (Grid, Int) = {
      if (steps == 0) grid -> totalFlashes
        ...
        val (newGrid, flashes) = octopusFlash(Grid(updatedGrid), totalFlashes)
        loop(newGrid, steps - 1, flashes)
      }
    }
