...
you would have a horizontal position of 15 and a depth of 60. (Multiplying these produces 900.)
...
object Solution {
  ...
  final case class Position(horizontal: Int = 0, depth: Int = 0, aim: Int = 0)

  def calculatePosition(input: List[Command]): Position = {
    input.foldLeft(Position()) { (position, command) =>
      command match {
  ...
  def calculatePositionWithAim(input: List[Command]): Position = {
    input.foldLeft(Position()) { (position, command) =>
      command match {
        case Command("forward", v) =>
          position.copy(
            horizontal = position.horizontal + v,
            depth = position.depth + (position.aim * v)
          )
        case Command("down", v) => position.copy(aim = position.aim + v)
        case Command("up", v)   => position.copy(aim = position.aim - v)
      }
    }
  }
}
